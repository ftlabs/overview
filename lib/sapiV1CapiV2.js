...
function intersectArrays( a=[], b=[] ){
	return a.filter(c => { return b.indexOf(c) > -1; });
}

// refactor into generic correlation a group selected from each article
function correlateGroupInArticles( fnGetGroupFromArticle, articles ){

	const correlations = {
		description : [
			'correlations: names appearing in the same articles',
			'correlations_1: names which are direct correlations of both name1 and name2',
			'correlations_2: names which are indirect correlations of both name1 and name2',
		],
		stats : {
		...
		directCorrelationsByName : {}, // [name] = [name, ...]
		correlations_1 : {}, // [name1][name2] = [name, ...],
		indirectCorrelationsByName: {}, // [name] = [name, ...]
		correlations_2 : {}, // [name1][name2] = [name, ...],
  ...
  const timings = {};
  timings['articles'] = { start : Date.now(), numAnnotations : 0, numArticles : articles.length }
	...
	articles.forEach( article => {
      ...
      timings.articles.numAnnotations += group.length
			...
			.map( it => { return [it, correlations.counts[it]]; } )
  ...
  timings.articles.end = Date.now();
  timings.articles.duration = timings.articles.end - timings.articles.start;

	correlations.stats.pubdateLatest   = pubdateLatest;
	correlations.stats.pubdateEarliest = pubdateEarliest;

	// compute correlations_1, names which are direct correlations of both name1 and name2
  timings['correlations_1'] = { start : Date.now(), numPairs : 0 , numPairsDeleted : 0 }
	const corrs   = correlations.correlations;
	const corrs_1 = correlations.correlations_1;
	const names   = Object.keys(corrs);

	// pre-calc direct correlations of each name
	const directCorrelationsByName = {};
	correlations['directCorrelationsByName'] = directCorrelationsByName;

	names.forEach( name => {
		directCorrelationsByName[name] = Object.keys( corrs[name] );
	} );
  const namesWithDirectCorrelations = names.filter( name => { return directCorrelationsByName[name].length > 0; });

	// find intersection of each pair of name's direct correlations
	namesWithDirectCorrelations.forEach( name1 => {
		corrs_1[name1] = {};
    const name1Correlations = directCorrelationsByName[name1];
		namesWithDirectCorrelations.forEach( name2 => {
			if (name1 === name2) { return; }
      timings.correlations_1.numPairs += 1;
			if (corrs_1.hasOwnProperty(name2) && corrs_1[name2].hasOwnProperty(name1)) {
				corrs_1[name1][name2] = corrs_1[name2][name1];
				return;
			}
			const name2Correlations = directCorrelationsByName[name2];
			corrs_1[name1][name2] = intersectArrays(name1Correlations, name2Correlations);
		});
	});

	// trim empty lists from corrs_1
	names.forEach( name1 => {
    if (! corrs_1.hasOwnProperty(name1)) {
      corrs_1[name1] = {};
    }
		Object.keys(corrs_1[name1]).forEach( name2 => {
			if (corrs_1[name1][name2].length == 0) {
				delete corrs_1[name1][name2];
        timings.correlations_1.numPairsDeleted += 1;
			}
		});
	});
  timings.correlations_1.end = Date.now();
  timings.correlations_1.duration = timings.correlations_1.end - timings.correlations_1.start;

	// compute correlations_2, names which are indirect correlations of both name1 and name2
  timings['correlations_2'] = { start : Date.now(), numPairs : 0 , numPairsDeleted : 0 }
	// pre-calc correlations of correlations of each names, i.e. the indirect correlations
	const indirectCorrelationsByName = {};
	correlations['indirectCorrelationsByName'] = indirectCorrelationsByName;

	names.forEach( name => {
		const corrCorrs = {};
		const nameCorrs = Object.keys(corrs[name]);
		nameCorrs.forEach(nameCorr => {
			const nameCorrCorrs = Object.keys(corrs[nameCorr]);
			nameCorrCorrs.forEach(nameCorrCorr => {
				if (! corrs[name][nameCorrCorr] && name !== nameCorrCorr) {
					corrCorrs[nameCorrCorr] = true;
				}
			});
		});
		indirectCorrelationsByName[name] = Object.keys(corrCorrs);
	});

  const namesWithIndirectCorrelations = names.filter( name => { return indirectCorrelationsByName[name].length > 0; });

	const corrs_2 = correlations.correlations_2;
	namesWithIndirectCorrelations.forEach( name1 => {
		corrs_2[name1] = {};
    const name1CorrCorrs = indirectCorrelationsByName[name1];
		namesWithIndirectCorrelations.forEach( name2 => {
			if (name1 === name2) { return; }
      timings.correlations_2.numPairs += 1;
			if (corrs_2.hasOwnProperty(name2) && corrs_2[name2].hasOwnProperty(name1)) {
				corrs_2[name1][name2] = corrs_2[name2][name1];
				return;
			}
			const name2CorrCorrs = indirectCorrelationsByName[name2];

			corrs_2[name1][name2] = intersectArrays(name1CorrCorrs, name2CorrCorrs);
		});
	});

	// trim empty lists from corrs_2
	names.forEach( name1 => {
    if (! corrs_2.hasOwnProperty(name1)) {
      corrs_2[name1] = {};
    }
		Object.keys(corrs_2[name1]).forEach( name2 => {
			if (corrs_2[name1][name2].length == 0) {
				delete corrs_2[name1][name2];
        timings.correlations_2.numPairsDeleted += 1;
			}
		});
	});
  timings.correlations_2.end = Date.now();
  timings.correlations_2.duration = timings.correlations_2.end - timings.correlations_2.start;

  debug(`correlateGroupInArticles: timings=${JSON.stringify(timings, null, 2)}`);
...
function correlateMergedAnnotations( params = {}, articles=[] ){
	...
	// debug(`correlateMergedAnnotations: defaultParams=${JSON.stringify(defaultParams)},
	// 	params=${JSON.stringify(params)},
	...
	combinedParams.groups.forEach( name => {
		if (groupFns.hasOwnProperty(name)) {
			correlations.groups[name] = correlateGroupInArticles( groupFns[name], articlesInGenres );
		}
	})
