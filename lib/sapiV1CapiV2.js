...
const concertinaOverlapThreshold = 0.5;
const concertinaNameJoin = ' + ';

function concertinaSortedListAndUpdateUuidGroups( sortedList, uuidsGroupedByItem ){
  ...
  while( sourceList.length > 1 ){
    ...
    const bubbleName  = bubble[0];
    const bubbleUuids = uuidsGroupedByItem[bubbleName];
    const bubbleCount = bubble[1];
    for (let i = sourceList.length-1; i>=0; i--) {
      candidate      = sourceList[i];
      candidateName  = candidate[0];
      candidateUuids = uuidsGroupedByItem[candidateName];
      ...
      if (overlapCount >= bubbleCount*concertinaOverlapThreshold) {
        // debug(`concertinaSortedListAndUpdateUuidGroups: overlapCount(${overlapCount}) >= bubbleCount(${bubbleCount})*concertinaOverlapThreshold(${concertinaOverlapThreshold})`);
        ...
        const mergedName  = [candidateName, bubbleName].join(concertinaNameJoin);
        const mergedItem  = [mergedName, mergedCount];
        ...
        while (insertJ >= 0 && sourceList[insertJ][1] < mergedCount) {
          insertJ -= 1;
        }
        ...
        // break out of for loop, back into while loop to pick up the end-most item (aka bubble)
        break;
  ...
  concertinaedList.sort( (a,b) => { if (a[1]<b[1]) { return 1; } else if (a[1]>b[1]) { return -1; } else { return 0; } });

  return concertinaedList;
}

function concertinaSortedLists( correlations ){
  // iterate over sortedByCount (and then over sortedByCountGroupedByTaxonomy)
  ...
  const concertinaedSortedLists = {
    sortedByCount : concertinaSortedListAndUpdateUuidGroups( correlations.sortedByCount, uuidsGroupedByItem),
    sortedByCountGroupedByTaxonomy : {},
  }

  taxonomies.forEach( taxonomy => {
    concertinaedSortedLists.sortedByCountGroupedByTaxonomy[taxonomy] = concertinaSortedListAndUpdateUuidGroups( correlations.sortedByCountGroupedByTaxonomy[taxonomy], uuidsGroupedByItem);
  });
...
function correlateGroupInArticles( fnGetGroupFromArticle, articles ){
	...
	articles.forEach( article => {
      ...
      correlations.concertinaedSortedLists = concertinaSortedLists(correlations);
