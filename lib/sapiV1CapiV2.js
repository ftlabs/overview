...
const concertinaOverlapThreshold = 0.75;
...
function concertinaSortedListAndUpdateUuidGroups( sortedList, uuidsGroupedByItem ){
  ...
  while( sourceList.length > 1 ){
    ...
    const bubbleConstituentNames = (bubble.length === 2)? [bubbleName] : bubble[2];
    ...
    for (let i = sourceList.length-1; i>=0; i--) {
      ...
      const candidateConstituentNames = (candidate.length === 2)? [candidateName] : candidate[2];
      ...
      if (overlapCount >= bubbleCount*concertinaOverlapThreshold) {
        ...
        const mergedItem  = [mergedName, mergedCount, mergedConstituentNames];
