...
function cachedFetchResText(path, apiKey, options) {
...
function cachedFetchResJson(path, apiKey, options){
	return cachedFetchResText(path, apiKey, options)
	.then(text => {
			let sapiObj;
			try {
...
function searchSapi(params) {
    ...
    return cachedFetchResJson(SAPI_PATH, apiKey, options)
        .then(json => {
            return {
                params,
                sapiObj: json
...
function getCapi(uuid) {
    ...
    return cachedFetchResJson(path, CAPI_KEY, options)
			.then(json => {
