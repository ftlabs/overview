...
function correlateGroupInArticles( fnGetGroupFromArticle, articles ){
	...
	articles.forEach( article => {
			...
			if (group.length == 0) {
				correlations.undefinedCount += 1;
				return;
			}

			// basic counts of each item
			group.forEach( item => {
...
function correlateMergedAnnotations( params = {}, articles=[] ){
	...
	// // count primaryTheme instances
	// const primaryThemesCounts = {};
	// let numNoPrimaryThemes = 0;
	// articlesInGenres.forEach( article => {
	// 	const primaryTheme = article.mergedAnnotations.primaryTheme;
	// 	if (primaryTheme === undefined) {
	// 		numNoPrimaryThemes++;
	// 	} else {
	// 		if (!primaryThemesCounts.hasOwnProperty(primaryTheme)) { primaryThemesCounts[primaryTheme] = 0; }
	// 		primaryThemesCounts[primaryTheme] += 1;
	// 	}
	// });
	//
	// const primaryThemesSortedByCount = Object.keys(primaryThemesCounts)
	// .map( pt => { return [pt, primaryThemesCounts[pt]]; } )
	// .sort( (a,b) => { if (a[1]<b[1]) { return 1; } else if (a[1]>b[1]) { return -1; } else { return 0; } });
	//
	// const primaryThemesSortedByCountGroupedByTaxonomy = {};
	// primaryThemesSortedByCount.forEach( ptc => {
	// 	const taxonomy = ptc[0].split(':')[0];
	// 	if (!primaryThemesSortedByCountGroupedByTaxonomy.hasOwnProperty(taxonomy)) {
	// 		primaryThemesSortedByCountGroupedByTaxonomy[taxonomy] = [];
	// 	}
	// 	primaryThemesSortedByCountGroupedByTaxonomy[taxonomy].push(ptc);
	// });
	//
	// const aboutsCounts = {};
	// const correlationsBetweenAbouts = {}; // [a][b] = count
	// articlesInGenres.forEach( article => {
	// 	const abouts = article.mergedAnnotations.abouts;
	// 	abouts.forEach( a => {
	// 		if (!aboutsCounts.hasOwnProperty(a)) {
	// 			aboutsCounts[a] = 0;
	// 		}
	// 		aboutsCounts[a] += 1;
	//
	...
	// 			if (a !== b) {
	// 				if (true) {
	// 					if (!correlationsBetweenAbouts.hasOwnProperty(a)) {
	// 						correlationsBetweenAbouts[a] = {};
	// 					}
	// 					if (!correlationsBetweenAbouts[a].hasOwnProperty(b)) {
	// 						correlationsBetweenAbouts[a][b] = 0;
	// 					}
	// 					correlationsBetweenAbouts[a][b] += 1;
	// 				}
	// 			}
	// 		});
	// 	});
	// });
	//
	// const aboutsSortedByCount = Object.keys(aboutsCounts)
	// .map( a => { return [a, aboutsCounts[a]]; } )
	// .sort( (a,b) => { if (a[1]<b[1]) { return 1; } else if (a[1]>b[1]) { return -1; } else { return 0; } });
	//
	// const aboutsSortedByCountGroupedByTaxonomy = {};
	// aboutsSortedByCount.forEach( ac => {
	// 	const taxonomy = ac[0].split(':')[0];
	// 	if (!aboutsSortedByCountGroupedByTaxonomy.hasOwnProperty(taxonomy)) {
	// 		aboutsSortedByCountGroupedByTaxonomy[taxonomy] = [];
	// 	}
	// 	aboutsSortedByCountGroupedByTaxonomy[taxonomy].push(ac);
	// });
	//
	// const correlations = {
	...
	// 	numNoPrimaryThemes,
	// 	aboutsSortedByCountGroupedByTaxonomy,
	// 	aboutsCounts,
	// 	aboutsSortedByCount,
	// 	correlationsBetweenAbouts,
	// 	numDistinctPrimaryThemes: Object.keys(primaryThemesCounts).length,
	// 	primaryThemesSortedByCountGroupedByTaxonomy,
	// 	primaryThemesCounts,
	// 	primaryThemesSortedByCount,
	// };

	const correlations = {
		genres: combinedParams.genres,
		numArticlesInGenres: articlesInGenres.length,
		abouts : correlateGroupInArticles( a=>{ return a.mergedAnnotations.abouts}, articlesInGenres),
	}
