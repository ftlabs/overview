...
function cachedFetchResJson(url, options){
	return cachedFetchResText(url, options)
	.then(text => {
			let sapiObj;
			try {
...
function searchSapi(params) {
    ...
    return cachedFetchResJson(sapiUrl, options)
        .then(json => {
            return {
                params,
                sapiObj: json
...
function getCapi(uuid) {
    ...
    return cachedFetchResJson(capiUrl, options)
			.then(json => {
