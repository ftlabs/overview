...
async function getArticlesAggregationWithListHistory(days, facets, aspects, minCorrelation=2, timeslip=0, listName='uk-homepage-top-stories'){
  ...
  const uuids = listHistory[0].map(item => { return item.content_id; });

  // lookup/filter UUIDs in genre:genre:News.articlesByUuid
  const newsUuids = uuids.filter( uuid => {
  ...
  newsUuids.forEach( uuid => {
    ...
    const primaryThemeCsv = (primaryTheme)? ['primaryTheme', primaryTheme.term.taxonomy, primaryTheme.term.name].join(':') : "unknown:unknown:unknown";
  ...
  newsUuids.sort((a,b) => {
    if (a.initialPublishDateTime < b.initialPublishDateTime) { return 1; }
    else if(a.initialPublishDateTime > b.initialPublishDateTime) { return -1; }
    else { return 0; }
  });
