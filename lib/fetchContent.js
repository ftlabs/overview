...
function getArticlesAggregation(days, facets, aspects, minCorrelation=2)
{
	...
	return searchDeeper(params, 10)
		.then(results =>  extractArticles(results) )
		.then(articles => aggregateArticles(articles, facets, minCorrelation, days, params, filterByMetadataCsv=null) )
		.then(aggregation => {
			// look for all the distinct genre CSVs
			let allCsvs = Object.keys(aggregation.articlesByMetadataCsv);
			let allGenreCsvs = allCsvs.filter( csv => csv.startsWith('genre:') );
			// re-establish the full list of articles
			let articles = Object.values(aggregation.articlesByUuid);
			// construct an aggregation for each genre csv
			let aggregationsByGenre = {};
			allGenreCsvs.forEach( csv => {
				aggregationsByGenre[csv] = aggregateArticles(articles, facets, minCorrelation, days, params, filterByMetadataCsv=csv);
			});
			let articleCounts = {};
			allGenreCsvs.forEach(csv => {
...
function aggregateArticles(articles, facets, minCorrelation, days, searchParams, filterByMetadataCsv=null)
{
	...
	articles.forEach(article => {
		...
		if (filterByMetadataCsv !== null) {
			let tuplesMatchingFilter = facetTuples.filter( facetTuple => facetTuple.csv === filterByMetadataCsv );
