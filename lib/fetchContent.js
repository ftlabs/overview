...
function wrapResults(results, days){
	return {
		...
		articleCount: results.articleCount,
		breakdown: results.breakdown
	};
}


function sortArticlesIntoTopics(articles)
{
	// TODO
	// get relatedTopic counts
	// sort topics with top article counts to top
	// sort relatedTopics counts to top
	...
	// Get all topics covered by articles
	articles.forEach(article => {
		totalArticleCount++;
		var topics = article.metadata.topics;

		if(!!topics){
			topics.forEach(topic => {
				allTopics.push(topic.term.name);
			});
		}
	});

	uniqueTopics = array.unique(allTopics);
	//console.log(uniqueTopics);


	// Add all topics to final list
	...
	//console.log(topicsWithArticles);


	// Add articles to their matching topics (in final list)
	articles.forEach(article => {
		var topics = article.metadata.topics;
		...
		if(!!topics){
			topics.forEach(topic => {
				...
				let obj = topicsWithArticles.find(function (obj) {
					return obj.topicName === topic.term.name;
				});

				// article count
				obj.articleCount = obj.articleCount + 1;

				// article list
				let currentArticles = obj.articles;
				currentArticles.push(article);
				obj.articles = currentArticles;

				// related topics
				let topicHolder = [];
				let moarTopics = article.metadata.topics;

				moarTopics.forEach(topic => {
					if(topic.term.name != obj.topicName){
						topicHolder.push(topic.term.name);
					}
				});

				let mergedTopics = topicHolder.concat(obj.relatedTopics);
				let uniqueTopics = array.unique(mergedTopics);
				obj.relatedTopics = uniqueTopics;
	...
	return {
		breakdown: topicsWithArticles,
		topicCount: topicsWithArticles.length,
