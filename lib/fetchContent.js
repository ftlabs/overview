...
function getArticleTopics(days, aspects, facets)
{
	const date = time.getDatetimeRange('days', days, 0);
	const params = {
		queryString: `lastPublishDateTime:> ${date.next}`,
	...
	return searchDeeper(params, 10)
		.then(results =>  extractArticles(results) )
		.then(articles => sortArticlesIntoTopics(articles) )
		.then(topicArticle => wrapResults(topicArticle, days) )
		.catch(err => {
			throw err;
		});
}
...
function wrapResults(results, days){
	return {
		description: `List of topics, from the last ${days} days. Each topic has a list of articles it features in and a list of other topics listed by those articles`,
		topicTotal: results.topicCount,
...
function sortArticlesIntoTopics(articles, facetName = 'topics')
{
	// TODO
	// add people and organisations as search grouping (currently just topics)
	// include people and organisations too
	...
	articles.forEach(article => {
		let topics = article.metadata[facetName];

		if(!!topics){
			topics.forEach(topic => {

				let itemObj = facetWithArticles.find(function (obj) {
					return obj.topicName === topic.term.name;
				});

				if(itemObj === undefined){
					facetWithArticles.push({
						topicName: topic.term.name,
						articles: [],
						articleCount: 0,
						relatedTopicCount: [],
						relatedTopics: [],
					...
					itemObj = facetWithArticles.find(function (obj) {
						return obj.topicName === topic.term.name;
					});
				}
				...
				// related topics
				let articlesTopics = article.metadata.topics;

				
				articlesTopics.forEach(topic => {
					if(topic.term.name != itemObj.topicName){
						itemObj.relatedTopics.push(topic.term.name);
					}
				});

				//related people
				let articlesPeople = article.metadata.people;

				if(articlesPeople){
					articlesPeople.forEach(person => {
						if(person.term.name != itemObj.topicName){
							itemObj.relatedPeople.push(person.term.name);
						}
					});
				}

				//related orgs
				let articlesOrgs = article.metadata.organisations;

				if(articlesOrgs){
					articlesOrgs.forEach(org => {
						if(org.term.name != itemObj.topicName){
							itemObj.relatedOrgs.push(org.term.name);
						}
					});
				}
	...
	//count facets
	facetWithArticles.forEach(facet => {
		facet.relatedTopicCount = uniqueSort(facet.relatedTopics, "count");
		facet.relatedPeopleCount = uniqueSort(facet.relatedPeople, "count");
		facet.relatedOrgs = uniqueSort(facet.relatedOrgs, "count");
		delete facet.relatedTopics;
		delete facet.relatedPeople;
		delete facet.relatedOrgs;
	});


	//sort by most articles
	array.sortArray(facetWithArticles, "articleCount");


	return {
		breakdown: facetWithArticles,
		topicCount: facetWithArticles.length,
		articleCount: facetWithArticles
	};
} 

function relatedFacets(data, combine, exclude){
	if(combine){
		console.log(combine);
		combine.forEach(facet => {
			if(facet.term.name !== undefined && facet.term.name != exclude){
				combine.push(facet.term.name);
			}
		});
	}
	return combine;
...
function uniqueSort(arr, sortable){
	return array.sortArray(array.uniqueCount(arr), sortable);
}
