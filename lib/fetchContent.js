...
function getArticlesAggregation(days, facets, aspects)
{
	const date = time.getDatetimeRange('days', days, 0);
	aspects = checkMetaDataPresent(aspects);
	const params = {
	...
	return searchDeeper(params, 10)
		.then(results =>  extractArticles(results) )
		.then(articles => aggregateArticles(articles, facets) )
		// .then(article => wrapResults(article, days) )
		.catch(err => {
			throw err;
		});
...
function addMetadataTerm(grouping, metadataKey, term, uuid){
...
function aggregateArticles(articles, facets)
{
	let articleMap = {}; // UUID -> article
	...
	articles.forEach(article => {
		...
		let facetTuples = []; // [ [metadataKey, taxonomy, name], ... ]

		metadataKeys.forEach( metadataKey => {
			if (metadataKey === 'primarySection') {
				addMetadataTerm(articlesByFacetTaxonomyName, 'primarySection', metadata['primarySection'].term, uuid);
				facetTuples.push(['primarySection', metadata['primarySection'].term]);
			} else if (metadataKey === 'primaryTheme') {
				addMetadataTerm(articlesByFacetTaxonomyName, 'primaryTheme', metadata['primaryTheme'].term, uuid);
				facetTuples.push(['primaryTheme', metadata['primaryTheme'].term]);
			} else {
				metadata[metadataKey].forEach( wrappedTerm => {
					let term = wrappedTerm.term;
					addMetadataTerm(articlesByFacetTaxonomyName, metadataKey, term, uuid);
					facetTuples.push([metadataKey, term]);
				});
			}
		});

		// calc all correlations between metadata items
		facetTuples.forEach( facetTuple1 => {
			facetTuples.forEach( facetTuple2 => {
				if ( facetTuple1[0]          !== facetTuple2[0]
					|| facetTuple1[1].name     !== facetTuple2[1].name
					|| facetTuple1[1].taxonomy !== facetTuple2[1].taxonomy) {
						addFacetCorrelation( facetCorrelations, facetTuple1[0], facetTuple1[1], facetTuple2[0], facetTuple2[1]);
					}
			});
		});
	});
