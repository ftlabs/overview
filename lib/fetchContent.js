...
function getArticleTopics(days, facet, aspects, facets)
{
	const date = time.getDatetimeRange('days', days, 0);
	const params = {
		queryString: `lastPublishDateTime:> ${date.next}`,
	...
	return searchDeeper(params, 10)
		.then(results =>  extractArticles(results) )
		.then(articles => sortArticlesIntoFacets(articles, facet) )
		.then(topicArticle => wrapResults(topicArticle, days, facet) )
		.catch(err => {
			throw err;
		});
}

function wrapResults(results, days, facet){
	return {
...
function sortArticlesIntoFacets(articles, facetName = 'topics')
{
			...
			facets.forEach(facet => {
				...
				if(articlesTopics){
					articlesTopics.forEach(topic => {
						if(topic.term.name != itemObj.facetName){
				...
				if(articlesPeople){
					articlesPeople.forEach(person => {
						if(person.term.name != itemObj.facetName){
				...
				if(articlesOrgs){
					articlesOrgs.forEach(org => {
						if(org.term.name != itemObj.facetName){
	...
	facetWithArticles.forEach(item => {
		item.relatedTopicCount = arrs.uniqueSort(item.relatedTopics, "count");
		item.relatedPeopleCount = arrs.uniqueSort(item.relatedPeople, "count");
		item.relatedOrgsCount = arrs.uniqueSort(item.relatedOrgs, "count");
		item.articleCount = item.articles.length;
		delete item.relatedTopics;
		delete item.relatedPeople;
		delete item.relatedOrgs;
	});
