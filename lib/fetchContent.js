	...
	articles.forEach(article => {
		...
		if(topics && topics !== undefined){
			topics.forEach(topic => {
				...
				let name = topic.term.name;
				let tempList = [];
				
				tempList.push(article);

				var obj = topicsWithArticles.find(function (obj) { return obj.name === name; });

				if(obj && obj.articles.length >= 1){

					let currentArticles = obj.articles;
					currentArticles.push(article);
					tempList = currentArticles;

					obj.articleCount = obj.articleCount + 1;
					obj.articles = tempList;
