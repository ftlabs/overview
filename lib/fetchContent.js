...
function sortArticlesIntoTopics(articles, facetName = 'topics')
{
	...
	articles.forEach(article => {
		let facets = article.metadata[facetName];

		if(!!facets){
			facets.forEach(topic => {

				let topicContainer = facetWithArticles.find(function (obj) {
					return obj.topicName === topic.term.name;
				});

				if(topicContainer === undefined){
					...
					topicContainer = facetWithArticles.find(function (obj) {
						return obj.topicName === topic.term.name;
					});
				}

				topicContainer.articleCount = topicContainer.articleCount++;
				topicContainer.articles.push(article);

				// related facets
				let articlesTopics = article.metadata.facets;

				articlesTopics.forEach(topic => {
					if(topic.term.name != topicContainer.topicName){
						topicContainer.relatedTopics.push(topic.term.name);
					}
				});
	...
	facetWithArticles.forEach(topic => {
		topic.relatedTopicCount = array.sortArray(array.uniqueCount(topic.relatedTopics), "count");
	});
...
function relatedFacets(facet, newFacets, existingFacets){
