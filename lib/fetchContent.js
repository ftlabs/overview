...
function wrapResults(results, days, facet){
	return {
		description: `List of ${facet}, from the last ${days} days. Each returned facet(${facet}) item has a list of articles it features in and a list of other facets listed by those articles`,
		facet: facet,
				...
				if(articlesTopics){
					articlesTopics.forEach(topic => {
						if(topic.term.name != itemObj.facetName){
							itemObj.related.topics.push(topic.term.name);
						}
					});
				}
				...
				if(articlesPeople){
					articlesPeople.forEach(person => {
						if(person.term.name != itemObj.facetName){
							itemObj.related.people.push(person.term.name);
						}
					});
				}
				...
				if(articlesOrgs){
					articlesOrgs.forEach(org => {
						if(org.term.name != itemObj.facetName){
							itemObj.related.orgs.push(org.term.name);
						}
					});
				}
