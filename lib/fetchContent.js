	...
	articles.forEach(article => {
		...
		if(!!facets){
			facets.forEach(facet => {
				...
				itemObj.related.people = addRelatedFacets('people', itemObj, article.metadata.people);
...
function addRelatedFacets(type, obj, data){
	if(data){
...
function searchFacetHistory(params)
{
	...
	return searchSequence(searches)
		.then(results => {
			...
			for(let i = 0; i <= intervaledFacets.length; i++){
				if(intervaledFacets[i]){
					intervaledFacets[i].forEach(item => {
						...
						if(!targetObj){
							let newCount = new Array(intervaledFacets.length).fill(0);
							collection.push({ name: name, count: newCount });
