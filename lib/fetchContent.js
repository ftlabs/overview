...
async function getRecentArticles(days){
	try{
		...
		const results = await searchDeeper(params, 10);
		let articles = [];

		results.forEach(result => {
			let collection = result.sapiObj.results[0].results;
...
async function searchFacetHistory(params){
	...
	try {
		...
		const queryResults = await searchSequence(searches);
