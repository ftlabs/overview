...
function aggregateArticles(articles, facets, minCorrelation=2)
{
	...
	articles.forEach(article => {
		...
		facetTuples.forEach( facetTuple => {
			addMetadataTerm(articlesByFacetTaxonomyName, facetTuple.metadataKey, facetTuple.term, uuid);
		});

		// construct facetCorrelations
		facetTuples.forEach( facetTuple1 => {
			facetTuples.forEach( facetTuple2 => {
				if ( facetTuple1.metadataKey   !== facetTuple2.metadataKey
					|| facetTuple1.term.name     !== facetTuple2.term.name
					|| facetTuple1.term.taxonomy !== facetTuple2.term.taxonomy) {
