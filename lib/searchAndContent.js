...
function calcFulldateRange(){
  const defaultEarliestDateTime = `${EARLIEST_YEAR}-01-01T00:00:00Z`;
...
function allFacets(params={}){
  ...
  const defaultParams = {
    queryString : calcFulldateRange().queryString,
    maxResults  : 1,
  ...
  .then(searchItem => {
    ...
    searchItem.lastPublishDateTime = {
      after : calcFulldateRange().after,
      before: calcFulldateRange().before,
    }
...
async function allFacetsByYear(){
  ...
  for (let year = EARLIEST_YEAR; year <= thisYear; year++) {
    ...
    const afterDataTime = `${year}-01-01T00:00:00Z`;
    const beforeDataTime = `${year+1}-01-01T00:00:00Z`;
    const params = { queryString: `lastPublishDateTime:>${afterDataTime} and lastPublishDateTime:<${beforeDataTime}` };
